...
namespace Abacus.Bonus {
	public class Repl {
		...
		public static void Run(bool funcDefMode = false) {
			Repl   repl = new Repl();
			string input;
			int    result;
			...
			while (true) {
				...
				result = repl.Evaluate(input, funcDefMode);
				Console.WriteLine($"> {result}");
			}
		}

		public int Evaluate(string input, bool funcDefMode) {
			int? lastResult = new int();
			foreach (string expr in input.Split(';')) {
				List<Token> tokens      = Lexer.Lex(expr, false, funcDefMode);
